...
describe('Model Selection Logic', () => {
  ...
  beforeEach(() => {
    ...
    process.env = { ...originalEnv, OPENROUTER_API_KEY: 'test-key', MODEL: 'env-model' };
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        choices: [{ message: { content: JSON.stringify({
          score: 50, explanation: 'ok', 
          summary: 'sum', headline: 'head', skills: [],
  ...
  describe('Scoring Service', () => {
    it('should use scoring specific model when set', async () => {
      ...
      await scoreJobSuitability({ title: 'Test Job', jobDescription: 'desc' } as any, {});
  ...
  describe('Project Selection Service', () => {
    it('should use project selection specific model when set', async () => {
      ...
      await pickProjectIdsForJob({
        jobDescription: 'desc',
        eligibleProjects: [{ id: '1', name: 'p1', description: 'd1', summaryText: 'summary' } as any],
        desiredCount: 1
      });
